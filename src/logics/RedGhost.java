...
/**
 * This class is for the red ghost. It keeps the ghost's position, speed etc. We have created methods so the ghost can move, change to scared mode and collide with the player.
 * The red ghost chases after the player by changing direction towards the player if on the same x or y coordinate.
 *
 */
public class RedGhost extends Ghosts {
	...
	private KeyCode direction = KeyCode.RIGHT;
	...
	public RedGhost() {
		super();
		try {
			this.normalGraphic = new Image(new FileInputStream("blinky.png"), SCREEN_WIDTH / 30, SCREEN_HEIGHT / 30,
					false, false);
